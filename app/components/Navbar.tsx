...
export default function Navbar({ activePage = '' }: NavbarProps) {
  ...
  return (
    <>
      <nav className="fixed top-0 w-full z-50 bg-slate-900/70 backdrop-blur-xl border-b border-white/10 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 pr-5 py-3 flex items-center justify-between" style={{ paddingRight: 'max(1.25rem, env(safe-area-inset-right, 1.25rem))' }}>
          ...
          <div className="hidden md:flex items-center gap-6 text-xs font-medium tracking-widest uppercase text-white/70">
            ...
            <div className="relative group">
              ...
              <div className="absolute top-full left-0 mt-2 w-48 bg-slate-900/90 backdrop-blur-xl border border-white/10 rounded-lg py-2 shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 translate-y-1 group-hover:translate-y-0">
      ...
      {/* MOBILE MENU */}
      <div className={`fixed inset-0 z-40 bg-slate-950 backdrop-blur-md transition-all duration-500 md:hidden flex flex-col ${mobileMenuOpen ? 'opacity-100 visible' : 'opacity-0 invisible pointer-events-none'}`}>
        <div className="pt-20 px-8 flex-1 overflow-y-auto">
          <div className="flex flex-col gap-1 max-w-md mx-auto">
            {mobileLinks.map((link) => (
              <a
                ...
                className={`text-lg font-bold uppercase tracking-wider py-4 border-b border-white/10 transition-colors ${isActive(link.href === '/' ? 'home' : link.href.slice(1)) ? 'text-[#007CAF]' : 'text-white/80 active:text-[#007CAF]'}`}
              >
                {link.label}
              </a>
            ))}
          ...
          <div className="mt-8 flex flex-col gap-3 max-w-md mx-auto">
            <a href="https://arcofitgym.clubautomation.com/member-portal/locations/1/memberships" target="_blank" rel="noopener noreferrer" className="w-full bg-[#007CAF] active:bg-[#006494] text-white font-bold px-6 py-4 rounded-lg text-center tracking-wider uppercase text-sm transition-colors">
              Membership Options
            </a>
            <a href="tel:3465532726" className="w-full border border-white/20 active:bg-white/10 text-white font-bold px-6 py-4 rounded-lg text-center tracking-wider uppercase text-sm transition-colors">
              346-553-ARCO
            </a>
          </div>
        </div>
